...
namespace REPOWildCardMod.Patches
{
    ...
    public static class HurtColliderPatch
    {
        ...
        {
            if (hitObject.TryGetComponent<PlayerAvatar>(out PlayerAvatar player) && __instance.ignoreObjects.Count > 0 && __instance.ignoreObjects[0].TryGetComponent<CloverNecklace>(out CloverNecklace necklace) && (player.physGrabber.grabbedPhysGrabObject == necklace || player == necklace.lastHolder))
            {
                __result = false;
                return false;
            }
